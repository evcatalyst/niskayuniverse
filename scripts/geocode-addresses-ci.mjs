...
async function geocodeWithNominatim(address, town, zip) {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        'User-Agent':
          'Niskayuna-Service-Markers/1.0 (https://github.com/evcatalyst/niskayuniverse)',
      },
    })
...
async function geocodeSimplified(address, town, zip) {
  ...
  const simplified = address
    .replace(/\b(?:AVE|ST|RD|DR|LN|WAY|PL|CT|TER|BLVD|HWY)\s*(?:NO|NORTH|SOUTH|EAST|WEST)?\b/gi, '')
    .trim()
...
async function geocodeAllAddresses() {
  ...
  console.log(
    `Found ${ungeocodedMarkers.length} ungeocoded addresses out of ${markersData.length} total`
  )
  ...
  for (let i = 0; i < ungeocodedMarkers.length; i += BATCH_SIZE) {
    ...
    console.log(
      `Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(ungeocodedMarkers.length / BATCH_SIZE)} (${i + 1}-${Math.min(i + BATCH_SIZE, ungeocodedMarkers.length)})`
    )
    ...
    console.log(
      `Saving progress... (${successCount + fallbackCount} successful, ${failureCount} failed)\n`
    )
