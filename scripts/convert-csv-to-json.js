...
const records = lines.slice(1).map((line, index) => {
  ...
  record.last_verified = new Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000)
    .toISOString()
    .split('T')[0]
...
fs.writeFileSync(path.join(publicDataDir, 'items.json'), JSON.stringify(itemsData, null, 2))

fs.writeFileSync(path.join(publicDataDir, 'markers.json'), JSON.stringify(markersData, null, 2))
