...
async function parsePDF(pdfPath) {
  ...
  for (const line of lines) {
    ...
    const pattern =
      /^(\d+)\s+([A-Z\s]+?)Niskayuna\s+(\d{5})(Copper|Lead|Galvanized|Plastic|Unknown)(Copper|Lead|Galvanized|Plastic|Unknown)$/i
    ...
    if (match) {
      ...
      const altPattern =
        /(\d+)\s+([A-Z\s]+?)\s*(Niskayuna)?\s*(\d{5})?\s*(Copper|Lead|Galvanized|Plastic|Unknown)?\s*(Copper|Lead|Galvanized|Plastic|Unknown)?/i
